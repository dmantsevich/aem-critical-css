...
exports.prepareConfiguration = async (config) => {
  ...
  if (criticalCSS.type === CRITICAL_CSS_TYPES.TEMPLATE) {
    if (!criticalCSS.useAPIService && !criticalCSS.AEMCriticalCSSServiceDestination) {
      logger.throwFatal(`Missed <blue>criticalCSS.useAPIService</blue> or <blue>${criticalCSS.AEMCriticalCSSServiceDestination}</blue> options. It's required for <key>criticalCSS.type</key>=<value>CRITICAL_CSS_TYPES.TEMPLATE</value>`);
    }
      ...
      if (parsed.dir) {
        if (parsed.ext === '.js') {
          ...
          if (criticalCSS.useAPIService.indexOf('jcr_root') !== -1) {
            logger.throwError(`Invalid <blue>criticalCSS.useAPIService</blue> value. Path should be absolute (in JCR)`);
          }
        }
      }
    }
  ...
  const _RESOURCES_FOLDER = config.web.сlientlibResourcesPath || RESOURCES_FOLDER;
  config.web.publicClientlib = normalizeUrl(config.web.publicClientlib + _RESOURCES_FOLDER);

  config.web._localClientlib = join(config.AEM.projectRoot, config.web.localClientlib);
  config.web.localClientlib = join(config.web._localClientlib, _RESOURCES_FOLDER);

  config._AEMCriticalCSSService = (await readFile(join(__dirname, '../aem-critical-css/AEMCriticalCSSService.js'))).toString();
...
exports.resolveInjectionType = async (component) => {
  ...
  if (injectionType !== LINK && injectionType !== INLINE) {
    logger.throwError(`Invalid <key>injection type</key> option. Supports only: <value>${INLINE}</value>, <value>${LINK}</value>.`);
  }
