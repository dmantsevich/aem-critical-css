...
exports.resolveByGzipSize = function resolveByGzipSize(component) {
  ...
  logger.info(`<fn>injectionTypeAutoResolver</fn>: <path>${component.cssSourceFile}</path> <blue>gzip size</blue> is <filesize>${gzip}</filesize>. Injection type: <value>${type}</value>`);
  return type;
};
...
exports.getAttribute = function getAttribute(fileContent, attr) {
  const regPattern = new RegExp(`\\s${attr}="(.+?)"`, 'gm');
  const pattern = regPattern.exec(fileContent);
  if (pattern && pattern[1] && pattern[1].trim()) {
    return {
      pattern: pattern[0],
      value: pattern[1].trim(),
      attr
    };
  }
  return null;
...
exports.resolveJCRPath = (filePath, from) => {
  if (filePath[0] === '/') return filePath; // Absolute JCR path
  if (filePath.endsWith('.js')) {
    return exports.getJCRPath(path.resolve(path.dirname(from), filePath));
  }
  return filePath;
};