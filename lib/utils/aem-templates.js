...
async function processTemplate (config, file, fileContent) {
  ...
  if (!Object.keys(criticalCSSMap).length) {
    logger.fatal(`Could not parse your critical css definition in <path>${file}</path> (<blue>${sourceAttr}</blue> attributes). Please, <!>check your configuration, template & read documentation</!>.`);
    throw new Error(`Could not parse ${file}. Please, check your configuration, template & read documentation.`);
  }
