...
async function compileCSS (component) {
  ...
  logger.info(`Start compiling <path>${cssSourceFile}</path> to CSS. Format: <blue>${ext.toUpperCase()}</blue>.`);
  try {
    const compile = require(`./compiler${ext}`);
    const css = await compile(component);
    component.css = css.toString();
    if (component.css && component.css.trim()) {
      logger.info(`Compilation for <path>${cssSourceFile}</path> was <green>successful</green>. Output size: <blue>${filesize(component.css.length)}</blue>`);
    } else {
...
module.exports = async (component) => {
  ...
  await require('./postcss')(component); // go through postcss plugins
  return component;
};