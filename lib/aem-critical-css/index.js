...
exports.deployAEMCriticalCSSService = async (config) => {
  let useAPIService = config.criticalCSS.useAPIService || '';
  if (useAPIService.endsWith('AEMCriticalCSSService.js') && config.criticalCSS.type === CRITICAL_CSS_TYPES.TEMPLATE) {
    useAPIService = join(config.AEM.projectRoot, useAPIService);
...
use(function () {
  ...
  return { shouldInject: CSSLoadingService.shouldInject(this.css) };
      ...
      const result = config.criticalCSS.minUseAPIService ? UglifyJS.minify(code, { output: {comments: 'all'} }) : { code };
      if (result.error) {
