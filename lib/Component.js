  ...
  async writeCSSFile(criticalCSS) {
    if (criticalCSS.injectionType === INJECTION_TYPES.LINK) {
      ...
      try {
        ...
        try {
          await fs.promises.access(cssTempPath, fs.constants.F_OK);
          logger.warn(`CSS file <path>${cssTempPath}</path> already exists. File will be overwritten.`);
        } catch (e) { }
        await writeFile(cssTempPath, criticalCSS.css);
        this.config._cssWasSavedAsFile = true;
      } catch (e) {
        logger.error(`Could not create <path>${cssTempPath}</path>. ${e.message}`);
        throw e
      }
    }
  }
