...
class Component {
  ...
  async writeAEMCriticalCSSFile() {
    try {
      ...
      logger.info(`Writing <path>${aemCriticalCSSFilePath}</path>...`);
  ...
  async writeCSSFile() {
    if (this.injectionType === INJECTION_TYPES.LINK) {
      const localClientlib = this.config.web.localClientlib;
      const cssTempPath = `${localClientlib}/${this.publicCSSFilename}`;
      try {
        logger.info(`Creating css file <path>${cssTempPath}</path>...`);
        await mkdir(dirname(cssTempPath), { recursive: true });
        await writeFile(cssTempPath, this.css);
        this.config._cssWasCreatedAsFile = true;
      } catch (e) {
